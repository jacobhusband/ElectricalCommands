    ...
    public partial class SimplerCommands
    {
        ...
        public static void RemoveRemainingXrefs()
        {
            ...
            try
            {
                ...
                // --- Part 3: Detach ALL PDF References ---
                using (var tr = db.TransactionManager.StartTransaction())
                {
                    // Step A: Erase all PdfReference entities
                    ...
                    foreach (ObjectId btrId in bt)
                    {
                        ...
                        if (btr.IsLayout)
                        {
                            ...
                            {
                                if (entId.ObjectClass.Name == "AcDbPdfReference")
                                {
                    ...
                    foreach (var pdfId in pdfsToErase)
                    {
                        try
                        {
                            var pdfRef = tr.GetObject(pdfId, OpenMode.ForWrite) as PdfReference;
                            if (pdfRef != null && !pdfRef.IsErased)
                            {
                                var layer = (LayerTableRecord)tr.GetObject(pdfRef.LayerId, OpenMode.ForRead);
                                bool relock = false;
                                if (layer.IsLocked)
                                {
                                    layer.UpgradeOpen();
                        ...
                        {
                            ed.WriteMessage($"\nCould not erase PDF reference {pdfId}: {ex.Message}");
                        }
                    }

                    // Step B: Remove all PDF definitions
                    var nod = (DBDictionary)tr.GetObject(db.NamedObjectsDictionaryId, OpenMode.ForRead);
                    string pdfDictKey = UnderlayDefinition.GetDictionaryKey(typeof(PdfDefinition));
                    if (nod.Contains(pdfDictKey))
                    {
                        ...
                        foreach (var kvp in defsToRemove)
                        {
                            ...
                            {
                                ed.WriteMessage($"\nFailed to remove PDF definition '{kvp.Key}': {ex.Message}");
                            }
                        }
                    }
                    tr.Commit();
